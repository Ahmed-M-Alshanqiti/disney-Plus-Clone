...
import serviceAccount from './disneyplus-clone-1342b-firebase-adminsdk-fbsvc-7c0806537a.json' assert { type: 'json' }; // Replace with your service account key
...
Object.keys(moviesData.movies).forEach((key) => {
    const movie = moviesData.movies[key];
    moviesRef.doc(key).set(movie)
        .then(() => {
            console.log(`Document ${key} added successfully!`);
        })
        .catch((error) => {
            console.error(`Error adding document ${key}:`, error);
        });
});
